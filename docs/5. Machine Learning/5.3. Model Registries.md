...
It can be used locally, however this means storing all models in the repository which will become quite large and it is also difficult to work with multiple analysts. 

Therefore it is better to use a hosted MLFlow server allowing multiple analysts to use the same registry, keeping the repository just for code.

MLFlow is cloud-agnostic and can be self-hosted on-premise, on cloud platforms or managed services such as Databricks.

## Model Logging

The experiment is based off the quickstart guide in the MLFlow documentation and is run after the model is trained. There is also the option of autologging experiments as models are trained.
...
- Features used
- Data used
- Signature (schema for input and output of model)
...
After experiments have been run, models can be registered to start managing the lifecycle.

```python
mlflow.register_model(name="catboost_frequency_model", model_uri=model_info.model_uri)
```
